...
class GolonganStockController extends Controller
{
    ...
    function data(Request $request)
    {
        ...
        try {
            // dd($vaRequestData);
            $nLimit = 10;
            $vaData = DB::table('golonganstock as r')
                ->select(
                    'r.KODE',
                    'r.KETERANGAN'
                );
    ...
    function store(Request $request)
    {
        try {
            ...
            $vaValidator = Validator::make($request->all(), [
                'KODE' => 'required|max:4|unique:golonganstock,KODE',
            ...
            $golonganStock = GolonganStock::create([
                'KODE' => $kode,
    ...
    function update(Request $request)
    {
        try {
            ...
            $golonganStock = GolonganStock::findOrFail($KODE);
            $golonganStock->KETERANGAN = $request->KETERANGAN;
            $golonganStock->save();
    ...
    {
        try {
            $golonganStock = GolonganStock::findOrFail($request->KODE);
            $golonganStock->delete();
