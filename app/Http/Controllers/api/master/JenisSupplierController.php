...
class JenisSupplierController extends Controller
{
    ...
    function data(Request $request)
    {
        ...
        try {
            // dd($vaRequestData);
            $nLimit = 10;
            $vaData = DB::table('jenissupplier as r')
                ->select(
                    'r.KODE',
                    'r.KETERANGAN'
                );
    ...
    function store(Request $request)
    {
        ...
        $vaValidator = Validator::make($request->all(), [
            'KODE' => 'required|max:4|unique:jenissupplier,KODE',
        ...
        try {
            $jenissupplier = JenisSupplier::create([
                'KODE' => $kode,
    ...
    function update(Request $request)
    {
        try {
            ...
            $KODE = $request->KODE;
            $jenissupplier = JenisSupplier::where('KODE', $KODE)->update([
                'KETERANGAN' => $request->KETERANGAN
            ]);
    ...
    {
        try {
            $jenissupplier = JenisSupplier::findOrFail($request->KODE);
            $jenissupplier->delete();
